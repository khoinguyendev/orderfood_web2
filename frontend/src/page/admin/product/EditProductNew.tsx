...
import ImageMultiImageUpload from "../../../components/admin/ImageMultiImageUpload ";
...
const EditProductNew = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        reset({
          ...
          available: productData.available ? "true" : "false",
        });
      } catch (error) {
        console.log(error);
      } finally {
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      setLoadingBtn(true);
      const response = await axios.put(`${SERVER_HOST}/products/new/${id}`, product);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 gap-6 mt-4">
        ...
        <div>
          ...
          {imageUrl && imageUrl.map((src)=> <div className="relative w-60 my-3 ">
              <img src={src} alt="Uploaded" className="w-60 h-40 object-cover rounded" />
              <button type="button" className="absolute top-0 right-0 bg-red text-white text-xs rounded-full p-1" onClick={() => setImageUrl((pre)=>pre.filter(i=>i!=src))}>
                X
              </button>
            </div>)}
          </div>
